...
- `Sources/AIService.swift` — own-API SSE streaming client (`URLSession.bytes`, `data:` line parsing). `APIProvider` selects the wire format: OpenAI and OpenRouter share chat-completions; Anthropic uses its messages API (top-level system prompt, `content_block_delta` SSE chunks). Errors are `LocalizedError` so the UI can show them raw.
- `Sources/Managers/UpdateChecker.swift` — deliberately not Sparkle (needs signing keys, appcast, framework embedding); compares `UpdateChecker.repo`'s latest GitHub release tag against `CFBundleShortVersionString` and opens the release page.
